...
'''从尝试一个简单的复制任务开始。从一个小的词汇表中给定一组随机的输入符号，目标是产生回这些相同的符号。'''
...
def data_gen(V, batch_size, nbatches):
    ...
    for i in range(nbatches):
        # torch.randint(low,high,size)：返回一个张量，包含了从low到high之间的随机整数，包含low，不包含high
        # data的维度是(batch_size,10)
        data = torch.randint(1, V, size=(batch_size, 10))
        # 把data数据放到GPU上
        if torch.cuda.is_available():
            data = data.cuda()

        # 第一个元素是用来表示句子开始的特殊符号，因此这行代码的作用是将所有句子的开头设置为这个特殊符号
        data[:, 0] = 1
        # 因为是复制任务，所以src和tgt是一样的
        src = data.requires_grad_(False).clone().detach()
        tgt = data.requires_grad_(False).clone().detach()
        yield Batch(src, tgt, 0) # 调用生成器函数，返回一个Batch对象
...
def example_simple_model():
    ...
    for epoch in range(20):
        model.train() # 将模型设置为训练模式。在训练模式下，模型会计算梯度并更新参数。这些都是通过调用model.train()来完成的。
        run_epoch(
